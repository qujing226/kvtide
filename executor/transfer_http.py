...
def create_executor_app(
    service: ExecuteServiceImpl, *, max_wire_bytes: int | None = None
):
    limit = (
        max_wire_bytes
        if max_wire_bytes is not None
        else service.max_transfer_bytes + 64 * 1024
    )
    if limit <= 0:
    ...
    async def app(scope, receive, send):
        if (
            scope["type"] != "http"
            or scope.get("path") != "/kvtide.v1.ExecutorService/PushKV"
        ):
        ...
        compressed = any(
            key.lower()
            in {b"content-encoding", b"grpc-encoding", b"connect-content-encoding"}
            and value.strip().lower() not in {b"", b"identity"}
            for key, value in headers
        )
        content_types = [
            v.split(b";", 1)[0].strip().lower()
            for k, v in headers
            if k.lower() == b"content-type"
        ]
        # Framed gRPC compression is intentionally not part of this v1 receiver.
        if compressed or content_types not in (
            [b"application/proto"],
            [b"application/json"],
        ):
            body = json.dumps(
            ...
            if message["type"] == "http.request":
                ...
                if total > limit:
                    ...
                    raise ConnectError(
                        ...
                        "PushKV request body exceeds byte limit",
                    )
            return message
