...
class ExecuteServiceImpl(executor_connect.ExecutorService):
    ...
    async def get_runtime(
        ...
        return executor_pb2.GetRuntimeResponse(
            ...
            kv_compatibility_id=compatibility.kv_compatibility_id
            if compatibility
            else "",
            transfer_endpoint=self.cfg.runtime.transfer_endpoint
            if compatibility
            else "",
        )
    ...
    async def push_kv(
        ...
        async with self._cache_lock:
            ...
            try:
                if (
                    len(request.key_data) + len(request.value_data)
                    > self.max_transfer_bytes
                ):
                    raise ConnectError(
                        Code.RESOURCE_EXHAUSTED,
                        "KV transfer exceeds the configured byte limit",
                    )
                expected = transfer.payload_bytes(len(blocks))
                if (
                    len(request.key_data) != expected
                    or len(request.value_data) != expected
                ):
                    raise ConnectError(
                        Code.INVALID_ARGUMENT,
                        f"each KV payload must have exactly {expected} bytes",
                    )
                ...
                if isinstance(exc, torch.OutOfMemoryError):
                    raise ConnectError(
                        Code.RESOURCE_EXHAUSTED, "insufficient memory for KV import"
                    ) from exc
    ...
    def _validate_trigger(
        ...
        if (
            ...
            raise ConnectError(
                Code.INVALID_ARGUMENT, "invalid destination transfer endpoint"
            )
        return transfer
    ...
    def _validate_push(self, request: executor_pb2.PushKVRequest) -> KVTransferRuntime:
        ...
        if request.destination_executor_id != self.cfg.runner.executor_id:
            raise ConnectError(
                Code.FAILED_PRECONDITION, "destination executor ID mismatch"
            )
        transfer = self._validate_common_transfer(
            transfer_id=request.transfer_id,
            source_executor_id=request.source_executor_id,
        ...
        if any(
            block >= transfer.cache.num_blocks
            for block in request.destination_block_ids
        ):
    ...
    def _validate_common_transfer(
        ...
        *,
        ...
        if transfer is None:
            raise ConnectError(
                Code.UNIMPLEMENTED, "this runner does not support KV transfer"
            )
        if (
            not transfer_id
            or len(transfer_id) > 128
            or not source_executor_id
            or len(source_executor_id) > 128
            or source_runtime_epoch == 0
            or not destination_executor_id
            or len(destination_executor_id) > 128
            or destination_runtime_epoch == 0
        ):
            raise ConnectError(
                Code.INVALID_ARGUMENT, "transfer identity is missing or invalid"
            )
        if (
        ...
        if not blocks or len(blocks) != len(set(blocks)):
            raise ConnectError(
                Code.INVALID_ARGUMENT,
                "destination block IDs must be nonempty and unique",
            )
        ...
        if 2 * transfer.payload_bytes(len(blocks)) > self.max_transfer_bytes:
            raise ConnectError(
                Code.RESOURCE_EXHAUSTED, "KV transfer exceeds the configured byte limit"
            )
        return transfer
    ...
    def _ensure_healthy(self) -> None:
        if self._cache_failed:
            raise ConnectError(
                Code.FAILED_PRECONDITION, "KV cache failed; restart this executor"
            )
